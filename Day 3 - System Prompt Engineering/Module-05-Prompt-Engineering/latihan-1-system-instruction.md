...
Sebelum copy-paste prompt ke Claude, pahami struktur eksperimen yang akan dibuat. Tujuan: **mengukur** penghematan token secara empiris, bukan sekadar percaya teori.

📂 **File baru**: `experiments/token-comparison.ts` (script eksperimen, bukan bagian app)

**1. Import SDK Anthropic + `ADVISOR_SYSTEM`**

📍 Lokasi: **paling atas file**.

```ts
// experiments/token-comparison.ts — bagian import
import Anthropic from "@anthropic-ai/sdk";
import { ADVISOR_SYSTEM } from "../src/features/prompts";
```

> 💡 Pakai **relative path** (`../src/...`) — bukan alias `@/...` — supaya tsx tidak perlu setup tsconfig path resolution.

**2. Siapkan 5 pertanyaan dummy untuk simulasi percakapan**

📍 Lokasi: **module level**, array string.

```ts
// experiments/token-comparison.ts — module level
const QUESTIONS = [
  "Tips menabung untuk DP rumah dalam 5 tahun",
  "Bagaimana cara mulai investasi pemula?",
  "Berapa idealnya dana darurat?",
  "Apa beda saham dan reksadana?",
  "Bagaimana atur budget bulanan?",
];
```

**3. Function `runScenarioA` — gaya Module 04 (prefixing)**

📍 Lokasi: **module level**, async function. Loop kirim 5 pertanyaan, **tanpa** parameter `system`, dengan `INSTRUCTION_PREFIX` di-concat ke setiap user message. Akumulasi `input_tokens` + `output_tokens` dari `response.usage`.

**4. Function `runScenarioB` — gaya Module 05 (system parameter)**

📍 Lokasi: **module level**, async function. Loop kirim 5 pertanyaan dengan `system: ADVISOR_SYSTEM`, user message **murni**. Akumulasi token yang sama.

**5. `main()` — jalankan keduanya, print tabel ASCII**

📍 Lokasi: **module level**, wrap di `main().catch(...)` karena CommonJS tidak support top-level await.

```ts
// experiments/token-comparison.ts — bagian bawah file
async function main() {
  const a = await runScenarioA();
  const b = await runScenarioB();
  console.log("| Scenario | Input | Output | Total |");
  console.log("|----------|-------|--------|-------|");
  console.log(`| A (prefix) | ${a.input} | ${a.output} | ${a.input + a.output} |`);
  console.log(`| B (system) | ${b.input} | ${b.output} | ${b.input + b.output} |`);
}
...
- ❌ Modifikasi `route.ts` atau `prompts.ts` — script ini **read-only** untuk pengukuran.
- ❌ UI atau integrasi ke app — file ini hanya untuk dijalankan via `tsx` di terminal.
- ❌ Persist hasil ke DB — cukup print ke stdout.
- ❌ Library tabel ASCII fancy — `console.log` dengan `|` separator sudah cukup.

### Verifikasi setelah file dibuat

1. Jalankan: `npx tsx --env-file=.env.local experiments/token-comparison.ts`.
2. Output muncul tabel dengan dua baris (A vs B) dan kolom Input/Output/Total tokens.
3. Skenario B (system) menggunakan **lebih sedikit total token** di 5-turn — biasanya 15-30% lebih hemat karena system prompt hanya dihitung sekali (saat caching aktif) atau lebih ringkas tiap turn.
4. Tidak ada error 401/429 — kalau ada, cek `.env.local`.
