...
| 3  | **[Role, Context, & Instruction](./latihan-3-rci.md)** | Refactor RCI + fitur Mode Perencanaan (2 prompt) | 40–50 menit |
...
Kerjakan Section 1–5 secara berurutan (**alur utama**) — tiap section mengandalkan output section sebelumnya. Pada akhir Section 5, AI Financial Advisor Anda akan memiliki **5-step reasoning workflow** + **fitur quick-add transaksi natural language** (Claude mem-parse "ngopi 5000" → JSON terstruktur → insert ke Supabase) yang menjadi fondasi paradigma **agentic AI** di module berikutnya.

**Section 2b** adalah **selingan opsional** setelah Section 2 — mengaktifkan teknik **zero-shot vs few-shot** dari Day 1 pada Claude API + Fin-App. Kerjakan bila Anda ingin memperkuat intuisi memilih teknik prompting **sebelum** menghadapi keputusan design di Section 5 (field `category` di tool `save_transaction`). Boleh dilewati saat pertama kali menyusuri modul dan dikerjakan setelah Section 5 selesai.
